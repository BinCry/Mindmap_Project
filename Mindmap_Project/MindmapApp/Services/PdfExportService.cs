    ...
    public class PdfExportService
    {
        public void ExportPdf(string filePath, string title, IEnumerable<NodeViewModel> nodes, IEnumerable<ConnectionViewModel> connections, string author, bool showWatermark = true, string? password = null)
        {
            ...
            try
            {
                ...
                {
                    var nodeBitmap = RenderNodeBitmap(node);
                    if (nodeBitmap == null) continue;

                    var xImage = CreateXImage(nodeBitmap, imageStreams);
                    gfx.DrawImage(xImage, node.X, node.Y, node.Width, node.Height);
                }
            ...
            {
                foreach (var stream in imageStreams) stream.Dispose();
            }
        }
        ...
        private static BitmapSource? RenderNodeBitmap(NodeViewModel node)
        {
            double width = Math.Max(1, node.Width);
            double height = Math.Max(1, node.Height);

            var visual = new DrawingVisual();
            using (var dc = visual.RenderOpen())
            {
                var rect = new Rect(0, 0, width, height);
                var bgBrush = new SolidColorBrush(node.BackgroundColor);
                var borderBrush = new SolidColorBrush(node.BorderColor);
                var borderPen = new Pen(borderBrush, 2);

                Geometry geometry = CreateNodeGeometry(node.Shape, rect);
                dc.DrawGeometry(bgBrush, borderPen, geometry);

                var contentBitmap = RenderFlowDocumentBitmap(node, width, height, out var contentHeight);
                if (contentBitmap != null)
                {
                    double offsetY = Math.Max(0, (height - contentHeight) / 2);
                    dc.DrawImage(contentBitmap, new Rect(0, offsetY, width, Math.Min(height, contentHeight)));
                }
            }
        ...
        private static Geometry CreateNodeGeometry(string shape, Rect rect)
        {
            switch (shape)
            {
                ...
                case "Diamond":
                    return CreatePolygonGeometry(new[]
                    {
        ...
        private static Geometry CreatePolygonGeometry(Point[] points)
        {
        ...
        private static XImage CreateXImage(BitmapSource source, List<MemoryStream> streams)
        {
