...
namespace MindmapApp.ViewModels
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        private readonly MindmapCloudService _cloudService;
        ...
        // 🔥 THÊM MỚI: ID bản đồ hiện tại (để hiển thị và chia sẻ)
        [ObservableProperty] private string _currentMapId = string.Empty;

        private string _currentShareCode = string.Empty;
        ...
        public event EventHandler<NodeViewModel>? NodeLockRequested;
        public event EventHandler<NodeViewModel>? NodeUnlockRequested;
        #endregion
        ...
        // 🔥 THÊM MỚI: Danh sách người dùng Online
        public ObservableCollection<UserAccount> OnlineUsers { get; } = new();
        ...
        // 🔥 THÊM MỚI: Các Command cho Collaboration
        public ICommand CopyMapIdCommand { get; }
        public ICommand JoinMapCommand { get; }

        public MainViewModel(MindmapSearchService searchService, MindmapAiService aiService, MindmapStorageService storageService, MindmapCloudService cloudService, UserService userService, UserAccount currentUser)
        {
            ...
            // 🔥 Khởi tạo Command mới
            CopyMapIdCommand = new AsyncRelayCommand(CopyMapIdAsync);
            JoinMapCommand = new AsyncRelayCommand(JoinMapAsync);
        }

        // 🔥 THÊM MỚI: Logic Copy ID
        private async Task CopyMapIdAsync()
        {
            if (string.IsNullOrWhiteSpace(_currentShareCode))
            {
                var published = await PublishCurrentMapAsync();
                if (!published) return;
            }

            try
            {
                Clipboard.SetText(CurrentMapId);
                MessageBox.Show("Đã copy mã bản đồ: " + CurrentMapId, "Thông báo");
            }
            catch (Exception ex)
            {
                MessageBox.Show("Không thể copy mã share: " + ex.Message, "Lỗi");
            }
        }

        // 🔥 THÊM MỚI: Logic Join Map (Hiện tại chỉ thông báo)
        private async Task JoinMapAsync()
        {
            var dialog = new JoinMapDialog { Owner = Application.Current?.MainWindow };
            if (dialog.ShowDialog() != true) return;

            var code = dialog.ShareCode;
            if (string.IsNullOrWhiteSpace(code)) return;

            try
            {
                var shared = await _cloudService.FetchSharedMapAsync(code.Trim());
                if (shared == null)
                {
                    MessageBox.Show("Không tìm thấy bản đồ với mã này.", "Lỗi");
                    return;
                }

                var document = _storageService.DeserializeDocument(
                    shared.ContentJson,
                    shared.Id,
                    shared.OwnerId,
                    shared.Title,
                ...
                CurrentMapId = string.IsNullOrEmpty(document.ShareCode) ? document.Id.ToString() : document.ShareCode;
                _currentShareCode = document.ShareCode;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Không thể tham gia bản đồ: " + ex.Message, "Lỗi");
            }
        }

        // 🔥 THÊM MỚI: Logic thêm người online vào danh sách hiển thị
        public void AddOnlineUser(string email)
        {
            if (!OnlineUsers.Any(u => u.Email == email))
            {
                // Tạo user tạm để hiển thị Avatar
                var newUser = new UserAccount
                {
                    Email = email,
                    DisplayName = email.Split('@')[0],
                    // Random màu nếu chưa có thông tin đầy đủ
                    UserColor = "#" + new Random().Next(0x100000, 0xFFFFFF).ToString("X6")
                };
        ...
        public void RequestNodeLock(NodeViewModel node) => NodeLockRequested?.Invoke(this, node);

        public void RequestNodeUnlock(NodeViewModel node) => NodeUnlockRequested?.Invoke(this, node);
        ...
        public void LoadMindmap(MindmapDocument document, bool keepHistory = false)
        {
            ...
            _currentShareCode = document.ShareCode;
            CurrentMapId = string.IsNullOrEmpty(_currentShareCode) ? _currentDocumentId.ToString() : _currentShareCode;
        ...
        public async Task SaveProfileAsync(string currentPass, string newPass, string confirmPass)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(newPass))
                {
                    ...
                    var changeResult = await _userService.ChangePasswordAsync(_currentUser.Email, currentPass, newPass);
                    if (!changeResult.Success)
                    {
                        MessageBox.Show("Không thể đổi mật khẩu trên server. Vui lòng kiểm tra kết nối hoặc mật khẩu hiện tại.");
                        return;
                    }
                    if (changeResult.Hash != null && changeResult.Salt != null)
                    {
                        _currentUser.PasswordHash = changeResult.Hash;
                        _currentUser.PasswordSalt = changeResult.Salt;
                    }
                }
        ...
        private async Task<bool> PublishCurrentMapAsync()
        {
            try
            {
                var document = BuildDocumentSnapshot();
                var contentJson = _storageService.SerializeDocument(document);
                var payload = new MindmapCloudService.MindmapPayload(
                    document.Id,
                    document.OwnerId,
                    document.Title,
                    contentJson,
                ...
                var shareCode = await _cloudService.PublishMapAsync(payload);
                if (string.IsNullOrWhiteSpace(shareCode))
                {
                    MessageBox.Show("Không thể tạo mã share lúc này. Vui lòng thử lại.", "Lỗi");
                    return false;
                }

                _currentShareCode = shareCode;
                CurrentMapId = shareCode;
            ...
            {
                MessageBox.Show("Không thể chia sẻ bản đồ: " + ex.Message, "Lỗi");
                return false;
            }
        }
        ...
        private string CreateDefaultXaml(string text)
        {
            var safeText = SecurityElement.Escape(text) ?? string.Empty;
            return $"<FlowDocument xmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\"><Paragraph TextAlignment=\"Center\"><Run>{safeText}</Run></Paragraph></FlowDocument>";
        }
